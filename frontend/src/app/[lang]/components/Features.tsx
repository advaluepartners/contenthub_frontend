      ...
      {showLink && url && text && (
        <div>
          <Link
            ...
            className="inline-block px-4 py-2 mt-4 text-sm font-semibold text-white transition duration-200 ease-in-out bg-white rounded-lg hover:bg-gray-600"
          >
            {text}
          </Link>
        </div>
...
export default function Features({ data }: FeaturesProps) {
  return (
    <section className="bg-white dark:text-gray-900 m:py-12 lg:py-24">
      <div className="container mx-auto py-4 space-y-2 text-center">
        <h2 className="text-5xl font-bold">{data.heading}</h2>
        <p className="dark:text-gray-900">{data.description}</p>
      </div>
